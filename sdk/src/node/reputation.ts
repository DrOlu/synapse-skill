...
export const DEFAULT_REPUTATION_CONFIG: Required<ReputationConfig> = {
  kvBucket: "REPUTATION",
...
export class ReputationStore {
  ...
  private kv: any;
  private localCache = new Map<string, ReputationRecord>();
  ...
  async initialize(): Promise<this> {
    ...
    try {
      ...
      const ttlNs = BigInt(7 * 24 * 3600) * 1_000_000_000n;
  ...
  private async consumeTaskUpdates(sub: Subscription): Promise<void> {
    try {
      for await (const msg of sub) {
        try {
          const envelope = JSON.parse(textDecoder.decode(msg.data)) as Envelope;
  ...
  recordOutcome(
    ...
    outcome: "success" | "failure" | "timeout" | "skill_not_found" | "overloaded" | "rate_limited",
    latencyMs?: number
  ): void {
  ...
  private kvKey(agentId: string, skill: string): string {
  ...
  private async save(record: ReputationRecord): Promise<void> {
    this.localCache.set(
      this.cacheKey(record.agent_id, record.skill),
      record
    );
    if (this.kv) {
      try {
        await this.kv.put(
          this.kvKey(record.agent_id, record.skill),
  ...
  private emitPenalty(record: ReputationRecord): void {
    try {
      this.nc.publish(
        `mesh.event.reputation.penalty.${this.safeKey(record.agent_id)}.${this.safeKey(record.skill)}`,
        textEncoder.encode(
          JSON.stringify({
            v: "1.0.0",
            id: uuidv4(),
  ...
  async deleteRecord(agentId: string, skill: string): Promise<void> {
  ...
  async discoverRanked(
    ...
    for (const manifest of agents) {
      const skillScores: Record<
        string,
        { score: number; success_rate: number; avg_latency_ms: number; flagged: boolean }
      > = {};
      let sumScore = 0;
      let count = 0;
  ...
  async smartRequest(
    ...
    if (ranked.length === 0) {
      throw new Error(
        `No agents available for capability "${capability}" skill "${skill}"`
      );
    }

    let lastError: unknown;
    const attempts: { agent: string; success: boolean; latency_ms?: number }[] = [];
  ...
  async stats(): Promise<ReputationStats> {
  ...
  dump(): { entries: Array<{ key: string; record: ReputationRecord }> } {
    return {
 ...
 */
export async function createReputationServiceAgent(
  mesh: Synapse,
  config?: Partial<ReputationConfig>,
  agentName: string = "Reputation Service"
): Promise<{ store: ReputationStore; manifest: AgentManifest }> {
  ...
  (mesh as any).onRequest("discover-ranked", async (payload: any) => {
    const ranked = await store.discoverRanked(payload?.input || {});
    return { agents: ranked };
  });

  (mesh as any).onRequest("get-record", async (payload: any) => {
  ...
  (mesh as any).onRequest("clear-flag", async (payload: any) => {
