...
import type { Synapse, AgentManifest, Skill, Envelope, SynapseError } from "./index.js";
...
export class HTTPBridge {
  ...
  async startWebhook(): Promise<void> {
    ...
    app.post("/mesh/discover", async (req, res) => {
      try {
        const agents = await this.mesh.discover(req.body as any || {});
        res.json({ agents });
      } catch (err: any) {
    ...
    app.post("/mesh/request", async (req, res) => {
      const { agentId, skill, input, timeout } = req.body as Record<string, any>;
      if (!agentId || !skill) {
